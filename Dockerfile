...
# Installing Latest Hashicorp Terraform (v0.12.20)
RUN curl -s https://releases.hashicorp.com/terraform/0.12.20/terraform_0.12.20_linux_amd64.zip > terraform_0.12.20_linux_amd64.zip && \
    unzip terraform_0.12.20_linux_amd64.zip -d /bin && \
    rm -f terraform_0.12.20_linux_amd64.zip


# Installing Latest Hashicorp Packer (v1.5.2)
RUN cd /tmp \
    && wget -q https://releases.hashicorp.com/packer/1.5.2/packer_1.5.2_linux_amd64.zip \
    && unzip packer_1.5.2_linux_amd64.zip \
    && mv packer /usr/local/bin/ \
    && rm -rf packer_1.5.2_linux_amd64.zip
