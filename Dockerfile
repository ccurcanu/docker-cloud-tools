...
# Installing latest stable Golang Go (go1.15.7)
RUN cd /tmp &&  \
    wget -q https://dl.google.com/go/go1.15.7.linux-amd64.tar.gz && \
...
# Installing Latest Hashicorp Terraform (v0.14.6)
RUN curl -s https://releases.hashicorp.com/terraform/0.14.6/terraform_0.14.6_linux_amd64.zip > terraform_0.14.6_linux_amd64.zip && \
    unzip terraform_0.14.6_linux_amd64.zip -d /bin && \
    rm -f terraform_0.14.6_linux_amd64.zip


# Installing Latest Hashicorp Packer (v1.6.6)
RUN cd /tmp \
