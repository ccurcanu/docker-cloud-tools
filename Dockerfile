...
# Installing latest stable Golang Go (go1.14.7)
RUN cd /tmp &&  \
    wget -q https://dl.google.com/go/go1.14.7.linux-amd64.tar.gz && \
...
# Installing Latest Hashicorp Terraform (v0.13.0)
RUN curl -s https://releases.hashicorp.com/terraform/0.13.0/terraform_0.13.0_linux_amd64.zip > terraform_0.13.0_linux_amd64.zip && \
    unzip terraform_0.13.0_linux_amd64.zip -d /bin && \
    rm -f terraform_0.13.0_linux_amd64.zip


# Installing Latest Hashicorp Packer (v1.6.1)
RUN cd /tmp \
