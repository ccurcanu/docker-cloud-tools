...
# Installing latest stable Golang Go (go1.15.3)
RUN cd /tmp &&  \
...
# Installing Latest Hashicorp Terraform (v0.13.4)
RUN curl -s https://releases.hashicorp.com/terraform/0.13.4/terraform_0.13.4_linux_amd64.zip > terraform_0.13.4_linux_amd64.zip && \
    unzip terraform_0.13.4_linux_amd64.zip -d /bin && \
    rm -f terraform_0.13.4_linux_amd64.zip


# Installing Latest Hashicorp Packer (v1.6.4)
RUN cd /tmp \
    && wget -q https://releases.hashicorp.com/packer/1.6.4/packer_1.6.4_linux_amd64.zip \
    && unzip packer_1.6.4_linux_amd64.zip \
    && mv packer /usr/local/bin/ \
    && rm -rf packer_1.6.4_linux_amd64.zip
