...
**Important** to know is that this image is generated by a serverless compute unit instance (AWS Lambda function triggered by CloudWatch) whenever a new version of **Terraform** and **Packer** are released. **Ansible**, or even **Go** versions can be set by updating `internal/store.json` file manually by a new commit.
